...
class _WaterPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 深水层
    final deepColor = const Color(0xFF1A3A4A).withValues(alpha: 0.6 + progress * 0.3);
    ...
    final surfaceColor =
        const Color(0xFF2E6B8A).withValues(alpha: 0.5 + progress * 0.4);
    canvas.drawPath(wavePath, Paint()..color = surfaceColor);

    // 水面光泽
    if (progress > 0.1 && progress < 0.9) {
      final highlightPaint = Paint()
        ..color = const Color(0xFF8AC4DE).withValues(alpha: 0.15);
    ...
    for (var i = 0; i < bubbleCount; i++) {
      ...
      canvas.drawCircle(
        ...
        bubbleR,
        Paint()
          ..color = const Color(0xFF8AC4DE).withValues(alpha: bubbleAlpha)
